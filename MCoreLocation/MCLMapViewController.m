...
#import "MCLMapViewController.h"
#import "MCLMapTypeBarButtonItem.h"
#import <objc/runtime.h>
#import "CLLocationManager+MCL.h"
#import "MCLAnnotationSegue.h"
...
static void * const kMCLMapViewControllerContext = (void *)&kMCLMapViewControllerContext;
static NSString * const kDefaultAnnotationReuseIdentifier = @"Annotation";

@interface MCLMapViewControllerLayoutGuide : NSObject <UILayoutSupport>
...
@implementation MCLMapViewControllerLayoutGuide
-(CGFloat)length{
...
#if MCoreLocation_USE_PRIVATE_API == 1
...
@interface MCLMapViewController()

@property (nonatomic, strong, readwrite) MCLMapTypeBarButtonItem *mapTypeBarButtonItem;
...
@property (nonatomic, strong) MCLMapViewControllerLayoutGuide *mvcLayoutGuide;
@property (nonatomic, strong) NSMutableDictionary<NSNumber *, id<MKAnnotation>> *annotationsByIndex;

@end

@implementation MCLMapViewController
...
- (void)commonInit {
    ...
    self.mvcLayoutGuide = [[MCLMapViewControllerLayoutGuide alloc] init];
...
- (void)showDetailForAnnotation:(id<MKAnnotation>)annotation{
    ...
    @catch (NSException *exception) {
        NSLog(@"Warning you must hookup a custom segue to a detail view controller with class %@ and identifier %@", NSStringFromClass([MCLAnnotationSegue class]), MHShowAnnotationSegueIdentifier);
    }
}
...
- (BOOL)canPerformShowAnnotationDetailSegue
{
    ...
    dispatch_once(&onceToken, ^{
        ...
        NSArray *filteredArray = [segueTemplates filteredArrayUsingPredicate:[NSPredicate predicateWithFormat:@"identifier = %@ AND segueClassName = %@", MHShowAnnotationSegueIdentifier, NSStringFromClass(MCLAnnotationSegue.class)]];
...
- (void)viewDidLoad
{
    ...
    self.mapTypeBarButtonItem = [[MCLMapTypeBarButtonItem alloc] initWithMapView:self.mapView];
...
#else
        image = [UIImage imageNamed:@"UIButtonBarListIcon" inBundle:[NSBundle bundleForClass:MCLMapViewController.class] compatibleWithTraitCollection:nil];
#endif
...
- (MKAnnotationView *)mapView:(MKMapView *)mapView viewForAnnotation:(id<MKAnnotation>)annotation{
    ...
    if(!pin){
        ...
        if([self canPerformShowAnnotationDetailSegue]){
            ...
            UIImage *image = [[UIImage imageNamed:@"DisclosureArrow" inBundle:[NSBundle bundleForClass:MCLMapViewController.class] compatibleWithTraitCollection:nil] imageWithRenderingMode:UIImageRenderingModeAlwaysOriginal];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if(self.annotationsTablePresentationStyle == MCLannotationsTablePresentationStyleModal){
...
- (void)presentAnnotationsTable{
    ...
    else if(self.annotationsTablePresentationStyle == MCLAnnotationsTablePresentationStyleSheet){
