...
//In code change MasterViewController to be a subclass of MCLMapViewController and delete all table code.
//That's it!
...
// Choose 'annotation detail' segue and set the identifer showAnnotation, class MCLAnnotationSegue, kind Custom.
...
typedef NS_ENUM(NSInteger, MCLannotationsTablePresentationStyle) {
    MCLannotationsTablePresentationStyleModal,
    MCLAnnotationsTablePresentationStyleSheet
};

MCoreLocation_EXTERN NSString * const MHShowAnnotationSegueIdentifier; // The default is 'showAnnotation' so set that identifier in the storyboard manual segue.
MCoreLocation_EXTERN NSString * const MHAnnotationCellIdentifier; // To use a custom cell in the table view that is hooked up to the outlet use the identifier 'annotation'

@class MCLMapTypeBarButtonItem;

@interface MCLMapViewController : UIViewController<MKMapViewDelegate, UITableViewDataSource, UITableViewDelegate>
...
@property (nonatomic, strong, readonly) MCLMapTypeBarButtonItem *mapTypeBarButtonItem;
...
@property (nonatomic, assign) MCLannotationsTablePresentationStyle annotationsTablePresentationStyle;
